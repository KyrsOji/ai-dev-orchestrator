...
# Create host writable directories for the runner (outside the read-only /opt runtime)
LOG_DIR="${LOG_DIR:-/var/log/ai-dev-runner}"
RUN_BASE_DIR="${RUN_BASE_DIR:-/var/lib/ai-dev-runner/openhands-runs}"
for d in "$RUN_BASE_DIR" "$LOG_DIR"; do
  if [ -n "$d" ]; then
    echo "[install] Ensuring directory exists: $d"
    mkdir -p "$d"
    ...
    if getent passwd "openhands-runner" >/dev/null 2>&1; then
      chown "openhands-runner":"openhands-runner" "$d"
    else
      chown root:"$RUNNER_GROUP" "$d"
    fi
    chmod 0750 "$d"
  fi
done

  ...
  return (
    <div style={{ padding: 8, borderRadius: 6, border: '1px solid #f1f5f9', background: '#ffffff', minHeight: 240 }}>
      ...
      {Array.isArray(events) && events.length ? (
        <div className="messages" style={{ paddingTop: 4 }}>
          {events.map((e: any) => {
            if (!e) return null
            try {
              ...
              if (e.type === 'followup') return <div key={e.id || Math.random()} style={{ marginBottom: 8 }}><ConversationFollowupCard followup={e.followup} /></div>
